...
**MANDATORY FLOW**: Q-SPEC推論提示 → ユーザー確認 → SPEC作成 → 実装

#### 絶対遵守事項
- **推論なしのSPEC作成禁止**: ユーザー確認なしに直接SPEC作成してはならない
- **確認待ち必須**: 推論提示後は必ずユーザーレスポンスを待つ
- **例外なし**: "research"や"implement"指示でも推論提示から開始

#### 唯一の例外
- ユーザーが明示的に「proceed」「create spec **directly**」と指示した場合のみスキップ可

#### 違反防止
- 全リクエストで推論ベースインタビューが必須であることを常に意識する
